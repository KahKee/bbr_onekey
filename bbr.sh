        ...
        centos)
            if [ -n "$(_os_ver)" ]; then
                ...
                if [ "$(_os_ver)" -eq 6 ]; then
                    ...
                    _error_detect "rpm --import https://www.elrepo.org/RPM-GPG-KEY-elrepo.org"
					_info "为RHEL-7、SL-7或CentOS-7安装ELRepo:"
					_error_detect "yum install https://www.elrepo.org/elrepo-release-7.0-3.el7.elrepo.noarch.rpm"
					_info "安装稳定主线内核kernel-ml（ml=mainline）"
					_error_detect "yum --enablerepo=elrepo-kernel -y install kernel-ml"
					_info "服务器已经安装的内核："
					awk -F\' '$1=="menuentry " {print i++ " : " $2}' /etc/grub
					_info "选择引导的内核:(#注："0"对应上面list的编号0)"
					read kernel_number
